...
### 安全（v0.6.0）
- **BLE 指令代理**: 所有车辆指令强制入口 — 白名单风险分级，高风险指令须用户确认，冲突指令严格串行
- **APK 插件沙箱**: 独立 ClassLoader（DexClassLoader）隔离加载，插件无法触及宿主内部
- **插件签名验证**: 证书 SHA-256 指纹，白名单放行 / 自签名用户确认 / 异常拒绝
- **插件事件总线**: 插件间类型化发布-订阅，解耦跨插件通信
...
- [行为准则](.github/CODE_OF_CONDUCT_zh.md)
- [贡献指南](.github/CONTRIBUTING_zh.md)
- [安全策略](.github/SECURITY_zh.md)
